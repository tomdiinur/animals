package com.example.tom.tryveg.carousel;
/*
...
public class CarouselViewAdapter extends BaseAdapter {
    ...
    private List<Pet> mDocus, mDocusOrig;
    int m_w, m_h;
    
    public CarouselViewAdapter(Context c, List<Pet> docuList, int image_max_w, int image_max_h) {
    	m_w = image_max_w;
    	m_h = image_max_h;
    	mDocus = docuList;
    	mContext = c;
	...
	private Bitmap createReflectedImage(Bitmap bmp) {
     ...
     public View getView(int position, View convertView, ViewGroup parent) {
    	 // empty items
    	 if (mDocus.size() ==0){
    		 Bitmap empty = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.shop);
		 ...
		 Bitmap originalImage = BitmapFactory.decodeResource(mContext.getResources(),
				 mDocus.get(position).getDrawable());

//		 originalImage = makeTransparent(originalImage, Color.RED);
...
//    	 Bitmap originalImage = BitmapFactory.decodeFile(mDocus.get(position).getImgPath());
    	 ...
    	 CarouselViewItem scaIv = new CarouselViewItem(mContext, reflection, mDocus.get(position).getName(), m_w, m_h);
...
//    	        	mDocus = (ArrayList<Pet>) results.values;
